...
FROM transactions";
$outstandingLoans = $conn->query($loanQuery)->fetch_assoc()['outstanding_loans'];

$monthQuery = "SELECT COALESCE(SUM(amount), 0) AS month_deposits 
FROM transactions 
WHERE type='deposit' 
AND MONTH(transaction_date) = MONTH(CURDATE()) 
AND YEAR(transaction_date) = YEAR(CURDATE())";
$monthlyDeposits = $conn->query($monthQuery)->fetch_assoc()['month_deposits'];
...
$where = '';
if ($search !== '') {
    $where = "WHERE members.firstname LIKE '%$search%' OR members.lastname LIKE '%$search%' OR transactions.type LIKE '%$search%'";
}

// pagination
$countQuery = "SELECT COUNT(*) as total FROM transactions 
JOIN members ON members.member_id = transactions.member_id $where";
            ...
            <table>
                ...
                <?php while ($row = $result->fetch_assoc()) :
                    ...
                    <tr>
                        ...
                        <td class="amount">MK <?php echo number_format($row['amount']); ?></td>
                        <td class="amount">MK <?php echo number_format($balances[$member]); ?></td>
